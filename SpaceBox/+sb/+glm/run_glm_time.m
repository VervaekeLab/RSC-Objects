...
[lapsNo,no_timeBins_in_lap]=unique(lap);

time = [];
for i = 1:length(lapsNo)
    trialLength(i) = length(find(lap == lapsNo(i)));
    time  = [time 0:1/31:(trialLength(i)/31-1/31)];
end
...
%%  calculate LLH - varying hyperparameter P model
% create paramter struct
neuron = cell(numNeuron,1);

for i = 1:numNeuron
    
    timeBinned         = zeros(length(time),1);
    binGapsTime  = 1:Z(n,1):max(time);
    yCenter     = 1:length(binGapsTime);
    for t = 1:length(time)
        [~,ind] = min(abs(binGapsTime-time(t)));
        timeBinned(t) = ind;
    end
    
    % calculate Grids
    timeGrid = zeros(length(timeBinned),length(unique(timeBinned))+1);
    for l = 1: length(timeBinned); timeGrid(l,timeBinned(l)+1) = 1; end; timeGrid(:,1) = 1;
    nBinTime = size(timeGrid,2)-1;
    
    signal = deconv(i,:)';
    numFolds = 10;
    
    dt = 1/31;
    [neuron(i).testFit{n},neuron(i).trainFit{n},neuron(i).param{n}] = fit_model(timeGrid,dt,signal,numFolds, Z(n,2)); 
    
end

time_hyper_par.neuron = neuron;
save(fullfile(savePath,'time_hyper_par'),'-struct', 'time_hyper_par');
