...
params.max_peak_width = 50; % 50
params.min_peak_dist = 20;
params.n_peak = 4;
params.min_peak_width = 4; %this was 4
params.high_low_difference =  0.25;
params.reliability_max = 0.2;
params.reliability_min = 0.1;
...
%     peak_threshold = prctile(avg_map_smoothed,75);
...
%                     peakLoc_first = intersect(find(avg_map_smoothed>peak_threshold),23:43);
%                     peakLoc_second = intersect(find(avg_map_smoothed>peak_threshold),63:83);
% 
%                     reliability_first = sum(max(rmaps(:,peakLoc_first,r)')>0) / size(rmaps,1);
%                     reliability_second = sum(max(rmaps(:,peakLoc_second,r)')>0) / size(rmaps,1);
%                     
                    if max([reliability_first,reliability_second]) >= params.reliability_max
                        if min([reliability_first,reliability_second]) >= params.reliability_min

                            % Include cell as landmark cell
...
if length(lcs)>0
    if ~isempty(rmaps2)
        ...
        for r = 1:size(rmaps2,3)
            ...
            if reliability >= params.reliability_max 
                tcs = [tcs,r];
            end
        end
        ...
        lcs_stats = lcs_stats(find(~ismember(lcs,tcs)));
    end
end
