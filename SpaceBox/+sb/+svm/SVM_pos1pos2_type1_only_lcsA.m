...
d_data = [];
% sData.trail.trialStart = [1 sData.trials.trialStart];
% noTrials = 20:10:length(sData.trials.trialStart)-length(sData.trials.trialStart)/5;
deconvSignal=[];
...
% trials = sData.behavior.wheelLapDs-min(sData.behavior.wheelLapDs)+1;
...
posBinned   = ones(length(position),1);

posBinnedTemp            = ones(length(positionBinned),1);

idxFirst = find(positionBinned >= 42-8& positionBinned <= 47+8);
idxSecond = find(positionBinned >= 122-8 & positionBinned <= 128+8);
...
for k = 1:numFolds
    ...
    trainIdx = setdiff(1:size(deconvSignalBinnedTemp,2),testIdx);
    
    d_data.iter{k}.deconvTrain = deconvSignalBinnedTemp(:,trainIdx);
    d_data.iter{k}.deconvTest  = deconvSignalBinnedTemp(:, testIdx);
    %
    %
    %% find cells without signal and delete
    d_data.iter{k}.cellIdx = cellIdx;
    d_data.iter{k}.realPos_train = posBinned(trainIdx); YTrain =  posBinned(trainIdx)';
    d_data.iter{k}.realPos_test  = posBinned(testIdx);  YTest =  posBinned(testIdx)';
